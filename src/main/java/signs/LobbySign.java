...
public class LobbySign {
    ...
    public void update(Map map, boolean force) {
        ...
        Iterator<?> var4 = this.splegg
                .maps
                .c
                .maps
                .getStringList("Signs." + map.getName() + ".lobby")
                .iterator();
        while (var4.hasNext()) {
            ...
            if (force) {
                ...
                String[] array = new String[] {
                    splegg.getConfig().getString("Sings.Restarting.1"),
                ...
                if (game == null) {
                    ...
                    sign[0] = splegg.getConfig()
                            .getString("Sings.Format.1")
                            .replaceAll("%status%", this.getFancyStatus(game))
                            .replaceAll("%map%", map.getName())
                            .replaceAll("%count%/%maxcount%", this.getPlayers(game));
                    sign[1] = splegg.getConfig()
                            .getString("Sings.Format.2")
                            .replaceAll("%status%", this.getFancyStatus(game))
                            .replaceAll("%map%", map.getName())
                            .replaceAll("%count%/%maxcount%", this.getPlayers(game));
                    sign[2] = splegg.getConfig()
                            .getString("Sings.Format.3")
                            .replaceAll("%map%", map.getName())
                            .replaceAll("%status%", this.getFancyStatus(game))
                            .replaceAll("%count%/%maxcount%", this.getPlayers(game));
                    sign[3] = splegg.getConfig()
                            .getString("Sings.Format.4")
                            .replaceAll("%count%/%maxcount%", this.getPlayers(game))
                            .replaceAll("%status%", this.getFancyStatus(game))
                            .replaceAll("%map%", map.getName());
                }
